...
while True:
    ...
    if tracking == 0:
        ...
        # Start the white timer only once.
        if white_start_time == None:
            white_start_time = running_time()

        white_elapsed = running_time() - white_start_time

        if white_elapsed >= WHITE_FORWARD_DELAY:
            # After 1 second of only white, move forward.
            # Go = both green
            ...
            # Search left before the 1 second timeout.
            # Left turn = left blue, right green
            ct.set_car_light(left_light, 0, 0, 255)
            ct.set_car_light(right_light, 0, 255, 0)

            ct.set_motors_speed(-SEARCH_SPEED, SEARCH_SPEED)

        elif last_seen == 1:
            # Search right before the 1 second timeout.
            # Right turn = right blue, left green
            ...
            # No previous direction known, move forward slowly.
            # Go = both green
            ct.set_car_light(left_light, 0, 255, 0)
            ct.set_car_light(right_light, 0, 255, 0)
        ...
        ct.set_motors_speed(SLOW_SPEED, BASE_SPEED)
        ...
        ct.set_motors_speed(BASE_SPEED, SLOW_SPEED)

    elif tracking == 11:
        # Both sensors see black.
        # Usually thick line or intersection.
